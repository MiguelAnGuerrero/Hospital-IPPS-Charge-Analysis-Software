...
<div class = "row">
    ...
    <?php
        foreach ($drg as $row){
			...
			foreach($row as $item){
				if($i ==0){
					$l = Uri::base() . "index.php/hospital/action_drgdetails/" . $row["DRG_Number"] + $row['DRG_Definition'];
...
<div class = "col-4">
	...
	<ul class = "pagination justify-content-sm-center">
		<?php	
			if($start > 0){
				$prev_path = Uri::base() . 'index/hospital/drgdetails/' . max($start - 20, 0);
				echo '<li class = "page-item"><a class="page-link" href="' . $prev_path . '">Previous</a><li>';
			}
			?>
		<?php
			$next_path = Uri::base() . 'index/hospital/drgdetails/' . ($start + 20);
